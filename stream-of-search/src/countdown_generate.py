...
# countdown specific
parser.add_argument("--start_range", default=4, type=int)
parser.add_argument("--min_range", default=4, type=int)
parser.add_argument("--max_target", default=100, type=int)
parser.add_argument("--num_train_samples", default=500000, type=int)
...
if __name__ == "__main__":
    ...
    for split, target_nums in zip(splits, target_list):
        ...
        print(
            f"average zeros: start size 3: {sum(average_zeros[3])}, start size 4: {sum(average_zeros[4])}, start size 5: {sum(average_zeros[5])}"
        )
        print(
        ...
        print(
            f"average reward: start size 3: {(sum(average_reward[3]) / total_samples[3]) if total_samples[3] else None}, start size 4: {(sum(average_reward[4]) / total_samples[4]) if total_samples[4] else None}, start size 5: {(sum(average_reward[5]) / total_samples[5]) if total_samples[5] else None}"
        )

        os.makedirs(args.data_dir, exist_ok=True)
        with open(
            f"{args.data_dir}/{split}{args.offset}_b{args.start_range}_t{args.max_target}_n{args.num_train_samples}_{args.search}.json",
            "w",
        ) as f:
