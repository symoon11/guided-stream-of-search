    ...
    def convert_to_path(self, target, nums, operations):
        ...
        for operation in operations:
            ...
            if len(nums) == 1:
                ...
                node_index += 1
                node_name = ",".join(["0" for _ in range(node_index)])
                search_trace += f"Generated Node #{node_name}: {target}:{new_nums} Operation: {operation}\n"
                search_trace += f"Moving to Node #{node_name}\n"
